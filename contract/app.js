...
(function () {
  ...
  function init() {
    ...
    function render() {
      ...
      /* 조항별 동의 표시 — 전자서명이면 화면에서 받은 체크 결과를 찍고,
         종이 자필·빈 양식이면 빈 칸(□)으로 찍어 회원이 펜으로 체크하게 한다. */
    ...
    function applySigModeUI() {
      ...
      $('#sigHint').textContent = paper
        ? '서명란과 약관 동의란이 빈 칸으로 나갑니다. 출력 후 종이에 직접 서명·체크받으세요.'
        : '화면에서 손가락이나 펜으로 서명을 받습니다. 서명과 약관 동의 결과가 그대로 인쇄됩니다.';
      var b = $('#modeBadge');
    ...
    function syncConsent() {
      ...
      var bh = $('#mbBtnHint');
      bh.hidden = ok;
      bh.textContent = readToEnd
        ? '아래 버튼을 누르면 모든 항목에 한 번에 동의됩니다.'
        : '약관을 스크롤해서 모두 읽어 주세요.';
      bh.classList.toggle('done', readToEnd);
